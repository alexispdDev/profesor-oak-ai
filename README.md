...
- **User query rewriting** — every question passes through `rewrite_query()` (`llm.py`) before retrieval: one LLM call that clarifies vague shorthand ("pikachu evo" -> "What is Pikachu's evolution chain?") while explicitly instructed to leave named Pokémon/moves/places unchanged, so it can't silently swap one real name for another. Falls back to the original question unchanged if the rewrite call itself fails. Verified against the evaluation harness before and after adding it: retrieval accuracy stayed at 16/16 (100%), no regression.
- **Hybrid search** — not implemented. Vector search (Qdrant + Ollama embeddings) was built and evaluated earlier in the project, then removed once semantic search proved unnecessary for a fully structured dataset (see [Decisions and trade-offs](#decisions-and-trade-offs)); it was never combined with keyword search as a hybrid approach.
- **Document re-ranking** — not applicable. Retrieval here is exact/fuzzy name and keyword matching against structured SQL columns, not a ranked list of documents, so there's nothing to re-rank.
